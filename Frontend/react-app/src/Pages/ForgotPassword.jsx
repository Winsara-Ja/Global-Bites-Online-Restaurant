            ...
            <div className='p-5 max-w-lg mx-auto mb-20 bg-white  rounded-2xl'>
                ...
                <div>
                    <form className=' flex flex-col gap-4 ' onSubmit={handleSubmit}>
                        <input type="email" placeholder="Enter your email" value={email} onChange={handleChange} className='bg-slate-300 p-3 rounded-lg' />
                ...
                {forgotPasswordStatus === 'success' && <p className="text-green-600">Password reset link is sent to your email. Please check your email.</p>}
                {forgotPasswordStatus === 'error' && <p className="text-red-600">Failed to send reset password link to the email</p>}
            </div>
        </div>
        </>
    );
