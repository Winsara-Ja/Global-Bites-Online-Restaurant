    ...
    <Page size="A4" style={styles.page}>
      <View style={styles.table}>
        <View style={styles.tableRow}>
          ...
          <View style={styles.tableColHeader}>
            <Text>Signin Date</Text>
          </View>
        </View>
        {signinHistories.map((signinHistory, index) => (
          <View style={styles.tableRow} key={index}>
            <View style={styles.tableCol}>
              <Text>{signinHistory.username}</Text>
            </View>
            <View style={styles.tableCol}>
              <Text>{signinHistory.email}</Text>
            </View>
            <View style={styles.tableCol}>
              <Text>{new Date(signinHistory.signinDate).toLocaleString()}</Text>
            </View>
          </View>
        ))}
      </View>
    ...
    const fetchSigninHistory = async () => {
      try {
        const response = await fetch(
          "http://localhost:5000/signinhistory/history"
        );
        if (!response.ok) {
            ...
            <tbody>
              {paginatedHistories.map((signinHistory, index) => (
                <tr key={index}>
                  ...
                  <td>{new Date(signinHistory.signinDate).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="btn1">
            ...
            >
              {({ loading }) =>
                loading
                  ? "Loading document..."
                  : "Download Login history as a PDF"
              }
            </PDFDownloadLink>
          </div>
        </div>
